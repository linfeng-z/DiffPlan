    ...
    def _percolate_down(self, curr_idx):
        while curr_idx < self.heap_length:
            ...
            if child2 >= self.heap_length:
                maxchild = None
            if (maxchild is not None) and (self.heap[child1] > self.heap[child2]):
                minchild = child2
                maxchild = child1
            ...
            if (maxchild is not None) and (self.heap[maxchild] < self.heap[curr_idx]):
...
def dijkstra_dist(maze, mechanism, goal):
    ...
    dist = (
        np.zeros((mechanism.num_orient, maze.shape[0], maze.shape[1]))
        ...
        for y in range(maze.shape[0]):
            for x in range(maze.shape[1]):
                if (orient == goal[0]) and (y == goal[1]) and (x == goal[2]):
                    continue
                pq.insert(
                    (orient, y, x), mechanism.num_orient * maze.shape[0] * maze.shape[1]
                )
    ...
    while not pq.empty():
        ...
        # Update neighbors
        for n in mechanism.inv_neighbors_func(maze, p_orient, p_y, p_x):
...
def dijkstra_policy(maze, mechanism, goal, policy):
    ...
    while not pq.empty():
        ...
        predecessors = mechanism.inv_neighbors_func(maze, p_orient, p_y, p_x)
        for i in range(len(predecessors)):
            ...
            if (
                (succ_pred_pol[0] == p_orient)
                and (succ_pred_pol[1] == p_y)
                and (succ_pred_pol[2] == p_x)
            ):
                # Update value
                curr_to_n = val + 1
                if curr_to_n < dist[n[0]][n[1]][n[2]]:
