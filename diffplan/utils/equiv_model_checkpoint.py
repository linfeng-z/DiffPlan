...
class ModelCheckpoint(Checkpoint):
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        if self.dirpath == dirpath_from_ckpt:
            ...
            self.kth_best_model_path = state_dict.get(
                "kth_best_model_path", self.kth_best_model_path
            )
            self.kth_value = state_dict.get("kth_value", self.kth_value)
            self.best_k_models = state_dict.get("best_k_models", self.best_k_models)
            self.last_model_path = state_dict.get(
                "last_model_path", self.last_model_path
            )
        else:
            warnings.warn(
    ...
    def _save_checkpoint(self, trainer: "pl.Trainer", filepath: str) -> None:
        # LHY: fix equiv model checkpoint issue
        trainer.model.eval()
    ...
    def __init_ckpt_dir(
        self, dirpath: Optional[_PATH], filename: Optional[str]
    ) -> None:
    ...
    def check_monitor_top_k(
        self, trainer: "pl.Trainer", current: Optional[Tensor] = None
    ) -> bool:
        if current is None:
            return False
