...
class MPPlanningNetwork(nn.Module):
    def __init__(self, args):
        ...
        # Policy prediction layer (includes position information)
        self.pi = MessagePassingLayer(
            in_dim=num_out_q + 2,  # Q-values + position coordinates
            out_dim=num_out_pi,
    ...
    def _value_iterate(self, pos, feat, edge_index):
        ...
        for t in range(self.k - 1):
            ...
            # Concatenate reward and value for Q-function computation
            rv = torch.cat([r, v], dim=1)
            q = self.q(pos, rv, edge_index)

            # Value iteration: V(s) = max_a Q(s,a), keeping dimension for next iteration
...
class MessagePassingLayer(MessagePassing):
    def __init__(self, in_dim, out_dim, hidden_dim=128, edge_dim=0, aggr="add"):
        ...
        # Input dims: (2 * node_features + edge_features) -> hidden_dim -> input_dim
        ...
        # MLP `\phi` for updating node features after message aggregation
        # Implemented as a stack of Linear->BN->ReLU->Linear->BN->ReLU
        # Input dims: (2 * input_dim) -> hidden_dim -> output_dim
