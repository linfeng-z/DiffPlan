...
class MPPlanningNetwork(nn.Module):
    def __init__(self, args):
        ...
        num_actions = (
            2 if args.cont_action else 4
        )  # discrete (4 directions), continuous (2 dimensions)

        # TODO init group and g_space
        ...
        # TODO Note: now consider number of parameters to be the same (when using `latent_dim_factor='sqrt'`)
        # hidden size for Message Passing Layer self.h
...
class Equivariant2DMessagePassingLayer(MessagePassing):
    ...
    def message(self, pos_i, pos_j, feat_i, feat_j, edge_attr):
        ...
        if edge_attr is None:
            msg = esnn.tensor_directsum([pos_i, pos_j, feat_i, feat_j])
        else:
            msg = esnn.tensor_directsum([pos_i, pos_j, feat_i, feat_j, edge_attr])
