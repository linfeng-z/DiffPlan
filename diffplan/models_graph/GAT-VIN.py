...
class GATVINPlanningNetwork(nn.Module):
    def __init__(self, args):
        ...
        # Feature encoding layer using Graph Attention Network
        self.h = torch_geometric.nn.GATConv(
            in_channels=num_in_h, out_channels=num_out_h
        )

        # Reward layer using Graph Attention Network
        self.r = torch_geometric.nn.GATConv(
            in_channels=num_in_r,
            out_channels=num_out_r,
        )

        # Q-value layer using Graph Attention Network
        self.q = torch_geometric.nn.GATConv(
            in_channels=num_in_q,
            out_channels=num_out_q,
        )

        # Policy layer using standard Graph Convolution (final output layer)
        self.pi = torch_geometric.nn.GCNConv(
            in_channels=num_in_pi, out_channels=num_out_pi
        )

        # Softmax activation for policy (applied along feature dimension)
    ...
    def _value_iterate(self, pos, feat, edge_index):
        ...
        for t in range(self.k - 1):
            ...
            # Concatenate reward and value for Q-function input
            rv = torch.cat([r, v], dim=1)
            q = self.q(torch.cat([pos, rv], dim=1), edge_index)

            # Value iteration: V = max_a Q(s,a)
